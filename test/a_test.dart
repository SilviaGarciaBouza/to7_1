...
void main(){
  group('Pruebas del modelo BikeType.', (){
    ...
    test('Prueba BikeType paso de double a int',(){
        expect(BikeType.fromJson({'vehicle_type_id': '1', 'count' : 2.0}).count, 2);
    });
  });

  group('Prueba del modelo Station', (){
    test('Prueba Station datos reales', (){
      expect(Station.fromJson({'station_id': '1', 'name':'Name', 'capacity':8, 'num_bikes_available':2,'num_docks_available':4, 'last_reported':12334, 'vehicle_types_available':[]}).id,'1' );
      expect(Station.fromJson({'station_id': '1', 'name':'Name', 'capacity':8, 'num_bikes_available':2,'num_docks_available':4, 'last_reported':12334, 'vehicle_types_available':[]}).name,'Name' );
    });
     test('Prueba Station datos vacios', (){
      expect(Station.fromJson({'station_id': '1', 'name':'Name', 'capacity':8, 'num_bikes_available':2,'num_docks_available':4,  'vehicle_types_available':[]}).lastReported,0 );
      expect(Station.fromJson({'station_id': '1',  'capacity':8, 'num_bikes_available':2,'num_docks_available':4, 'last_reported':12334, 'vehicle_types_available':[]}).name,'name unknown' );
    });
    test('Prueba Station paso de double a int', (){
      expect(Station.fromJson({'station_id': '1', 'name':'Name', 'capacity':8, 'num_bikes_available':2.8,'num_docks_available':4,  }).numBikesAvailable,2 );
      expect(Station.fromJson({'station_id': '1',  'capacity':8, 'num_bikes_available':2,'num_docks_available': 4.0, 'last_reported':12334, }).numDocksAvailable,4 );
    });
  });
 

  group('Prueba viewModel', (){
    var vm= Stationviewmodel(repository: RespositoryTest(is_Successfull: true));
    var vmError =  Stationviewmodel(repository: RespositoryTest(is_Successfull: false));
